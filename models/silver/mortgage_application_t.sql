...
{{
	config(
        ...
        post_hook = ["{{ m_upd_post_load_attrib_vals_for_model_record_in_dbt_model_audit( var('p_pipeline_name'), 'silver_mcd', this , m_get_batch_id( var('p_pipeline_name') ) , 'src_load_ts' ) }}"]
    )
}}

select
    raw_json:application_id::string as application_id,
    raw_json:customer_id::string as customer_id,
    raw_json:application_date::date as application_date,
    raw_json:amount_requested::number(18,2) as amount_requested,
    raw_json:property_type::string as property_type,
    upper(raw_json:status::string) as status,
    src_load_ts,
    {{ v_batch_id }} as batch_id,
    current_timestamp() as etl_load_ts,
    current_timestamp() as etl_updt_ts
from {{ source('mortgage_src','mortgage_application_raw') }} src
	{% if is_incremental() %}
