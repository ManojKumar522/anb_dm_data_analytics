...
select
    raw_json:payment_id::string as payment_id,
    raw_json:application_id::string as application_id,
    raw_json:payment_date::date as payment_date,
    raw_json:payment_amount::number(18,2) as payment_amount,
    raw_json:payment_status::string as payment_status,
    src_load_ts,
    {{ v_batch_id }} as batch_id,
    current_timestamp() as etl_load_ts,
    current_timestamp() as etl_updt_ts
from {{ source('mortgage_src','mortgage_payment_raw') }} src
    {% if is_incremental() %}
