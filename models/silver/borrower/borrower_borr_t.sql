...
{{
    config(
        ...
        strategy = "merge",
        unique_key = ["loan_key", "borr_pstn_nbr", "sor_cd"],
        merge_exclude_columns = ["etl_load_ts"],
        post_hook = [
            "{{ m_upd_post_load_attrib_vals_for_model_record_in_dbt_model_audit(var('p_pipeline_name'), 'silver_mcd', this, m_get_batch_id(var('p_pipeline_name')), 'aud_upd_dttm') }}"
        ]
    )
}}
...
select
    ...
    sor_cd,
    aud_upd_dttm,
	aud_upd_dttm as src_load_ts,
    {{ v_batch_id }} as batch_id,
    current_timestamp() as etl_load_ts,
    current_timestamp() as etl_updt_ts
from {{ source('borrower_src', 'borr') }} src
{% if is_incremental() %}
    where src.aud_upd_dttm > {{ v_inc_load_ts }}
{% endif %}