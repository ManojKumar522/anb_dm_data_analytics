{% macro m_mysql_process_deletions_from_silver_using_model_id(p_pipeline_name, p_model_grp, p_model_id, p_batch_id) %}
    ...
    {% if execute %}
        {% set v_src_tbl = '' %}
        {% set v_tgt_tbl = '' %}
        {% set deletions_table = source('dbt_audit', 'dbt_deletion_log_t') %}
...
{% macro m_apply_typ1_deletion_on_tgt_model( p_src_tbl, p_tgt_tbl, p_src_key_column ) %}
    ...
    {% set delete_sql -%}    
        ...
        where {{ v_src_key_column }} in ( select distinct pk from {{ source('dbt_audit', 'dbt_deletion_log_t') }} s where upper(model_id) = upper('{{ v_src_tbl }}') );
        commit;
    {%- endset %}
...
{% macro m_apply_typ2_deletion_on_tgt_model( p_src_tbl, p_tgt_tbl, p_src_key_column ) %}
    ...
    {% set update_sql -%}    
        ...
        set active_flg = 'N',
            end_ts = etl_updt_ts
        where active_flg = 'Y'
            and {{ v_src_key_column }} in ( select distinct pk from {{ source('dbt_audit', 'dbt_deletion_log_t') }} s where upper(model_id) = upper('{{ v_src_tbl }}') );
        commit;
    {%- endset %}
